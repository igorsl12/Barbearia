...
export function AdminDashboard() {
  ...
  const { appointments, loading, loadByDate, updateStatus } = useAppointments()
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <Link to="/admin/history" className="p-2 hover:bg-gray-100 rounded-lg">
              <History size={18} className="text-gray-600" />
            </Link>
            <Link to="/admin/identity" className="p-2 hover:bg-gray-100 rounded-lg">
              <Palette size={18} className="text-gray-600" />
            </Link>
            <Link to="/admin/settings" className="p-2 hover:bg-gray-100 rounded-lg">
              <Settings size={18} className="text-gray-600" />
            </Link>
            <button onClick={signOut} className="p-2 hover:bg-gray-100 rounded-lg">
          ...
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-semibold text-gray-900">
              ...
              {appointments.length > 0 && (
                <span className="text-gray-400 font-normal">({appointments.length})</span>
              )}
            </h2>
            {!isToday(date) && (
              <button
